    ...
    <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#687080]">
                {myBalance.netBalance > 0.005 ? 'you are owed' : myBalance.netBalance < -0.005 ? 'you owe' : 'all settled Γ£ô'}
              </p>
            </div>
          </div>
        </div>
      ...
      {settlements && settlements.length === 0 && balances.length > 0 && (
        <div className="saas-card p-4 text-center">
          <p className="text-[#2F9B70] text-sm font-bold">Γ£ô All balances are settled!</p>
        </div>
      )}
    </div>
  )
